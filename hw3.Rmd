...
# # for piecewise, at what what quantity does the high_demand have a price equal to the low_demand
# 
# low_price = lm_low$coefficients[1]
# dem_high_q <-function(p) {q = lm_high$coefficients[1] + lm_high$coefficients[2]*q}
# 
# # this quantity = our change point
# 
# gas_df_agg <- gas_df_agg %>% 
#   mutate(diff = q_agg_gallons-change_point) %>% 
#   mutate(dummy = ifelse(q>=change_point, 1, 0)) %>% 
#   mutate(after_change_point = dummy*diff)
# 
# gas_ag_model_pw <- lm(price_dollars ~ q_agg_gallons + after_change_point, data = gas_df_agg)
